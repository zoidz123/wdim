...
- Fix packaged X / Twitter scanning by patching the bundled bird cookie reader for Electron's Node runtime, and show source auth/errors in recent scan summaries instead of flattening them to zero new items.
...
- Updated dependencies
  - @what-did-i-miss/shared@0.1.1

## 0.1.1

### Patch Changes

- Set up Changesets release versioning and use the desktop package version for app-server client metadata.
- Add explicit YouTube video sources with a manual Summarize action.
- Render YouTube summary timestamp links and preserve timed caption cues through scanning.
- Use the explicit `gpt-5.5` scan model for the bundled Codex app-server.
